...
```{r njt_data}
njt_ids <- read.delim("../data/njt_data/STATIONS_EXTRACT.TXT", sep = "\t")
...
#Faster but has a bug with carriage returns
# library(readr)
# njt_colpositions <- fwf_widths(widths = njt_widths,col_names = njt_colnames)
# njt_2010 <- read_fwf(file = "../data/njt_data/TRAIN_EXTRACT_2010.TXT", col_positions = njt_colpositions, col_types = "ciccccccc")
...
#POSIXct required date when providing time
...
```{r noaa_data}
weather_daily <- read.csv(file = "../data/weather_data/weather/NY_NJ-daily_weather.csv") %>% mutate(YearMonthDay = as.POSIXct(strptime(YearMonthDay, format = "%Y%m%d")))
