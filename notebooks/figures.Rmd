...
baseline <- feat_ex %>% count(is_delayed, Line) %>% spread(is_delayed, n) %>% mutate(pct_delays = Yes/ (No + Yes) * 100) %>% .$pct_delays

rbind(timetolast, byhour, bytemp, bywday) %>% ggplot(aes(feat, pct_delays)) + geom_bar(stat = "identity") + facet_wrap(~type, scale = "free") + geom_hline(yintercept=baseline, color = "red", size = 1, linetype = 2) + theme_minimal() + xlab("") + ylab("% of Trains Delayed")
