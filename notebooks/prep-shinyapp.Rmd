...
cpt_wday <- njt_features %>% 
  ...
  transmute(Line, feature = "dep_wday", feature_val = dep_wday, p_fold = log(pct/base_pct))
...
cpt_mon <- njt_features %>% 
  ...
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "dep_mon", feature_val = dep_mon, p_fold = log(pct/base_pct))
...
cpt_hour <- njt_features %>% 
  ...
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "dep_hour", feature_val = as.factor(dep_hour), p_fold = log(pct/base_pct))
```
...
cpt_WindSpeed <- njt_features %>% group_by(Line) %>%
  ...
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "WindSpeed", feature_val = as.factor(bin), p_fold = log(pct/base_pct))
...
cpt_Temp_F <- njt_features %>% group_by(Line) %>%
  ...
  mutate(Line, bin, pct = Yes/(No+Yes)*100) %>% 
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "Temp_F", feature_val = as.factor(bin), p_fold = log(pct/base_pct))
...
cpt_Visibility <- njt_features %>% group_by(Line) %>%
  ...
  mutate(Line, bin, pct = Yes/(No+Yes)*100) %>% 
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "Visibility", feature_val = as.factor(bin), p_fold = log(pct/base_pct))
```
...
cpt_ttl_line <- njt_features %>% group_by(Line) %>%
  ...
  mutate(Line, bin, pct = Yes/(No+Yes)*100) %>% 
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "ttl_line", feature_val = as.factor(bin), p_fold = log(pct/base_pct))
```
...
pred_cpt <- pred_df %>% mutate(
  ttl_line = cut(ttl_line, breaks_ttl_line , include.lowest = TRUE),
  Temp_F = cut(Temp_F, breaks_Temp_F , include.lowest = TRUE),
  WindSpeed = cut(WindSpeed, breaks_WindSpeed , include.lowest = TRUE),
  Visibility = cut(Visibility, breaks_Visibility , include.lowest = TRUE)
) %>% 
  select(Line, dep_hour:WindSpeed) %>% gather(feature, feature_val, -Line) %>% 
...
```{r}
pred_cpt  %>% ggplot(aes(feature,p_fold, fill = p_fold)) + geom_bar(stat = "identity") + facet_wrap(~Line, ncol = 4) + coord_flip() + theme_minimal() + scale_fill_gradient2(low = "blue", mid = "black", high = "red", midpoint = 0) + ylim(-1,1)
```
