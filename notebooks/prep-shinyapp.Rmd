...
cpt_wday <- njt_features %>% 
  ...
  transmute(Line, feature = "dep_wday", feature_val = dep_wday, p_fold = log2(pct/base_pct))
...
cpt_mon <- njt_features %>% 
  ...
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "dep_mon", feature_val = dep_mon, p_fold = log2(pct/base_pct))
...
cpt_hour <- njt_features %>% 
  ...
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "dep_hour", feature_val = as.factor(dep_hour), p_fold = log2(pct/base_pct))
```
...
cpt_WindSpeed <- njt_features %>% group_by(Line) %>%
  ...
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "WindSpeed_break", feature_val = as.factor(bin), p_fold = log2(pct/base_pct))
...
cpt_Temp_F <- njt_features %>% group_by(Line) %>%
  ...
  mutate(Line, bin, pct = Yes/(No+Yes)*100) %>% 
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "Temp_F_break", feature_val = as.factor(bin), p_fold = log2(pct/base_pct))
...
cpt_Visibility <- njt_features %>% group_by(Line) %>%
  ...
  mutate(Line, bin, pct = Yes/(No+Yes)*100) %>% 
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "Visibility_break", feature_val = as.factor(bin), p_fold = log2(pct/base_pct))
```
...
cpt_ttl_line <- njt_features %>% group_by(Line) %>%
  ...
  mutate(Line, bin, pct = Yes/(No+Yes)*100) %>% 
  inner_join(cpt_base, by = "Line") %>% 
  transmute(Line, feature = "ttl_line_break", feature_val = as.factor(bin), p_fold = log2(pct/base_pct))
```
